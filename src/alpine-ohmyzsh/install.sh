...
if [ -z "$ZSHPATH" ]; then
  ZSHPATH=$DEFAULTZSHPATH
fi

KEEP_ZSHRC=yes ZSH=$ZSHPATH su -c "wget -qO- https://raw.githubusercontent.com/ohmyzsh/ohmyzsh/master/tools/install.sh | sh -s" $_CONTAINER_USER

sed -i 's/plugins=(git)/plugins=(\n)/' $_CONTAINER_USER_HOME/.zshrc

if echo "$PLUGINS" | grep -w -q "zsh-autosuggestions"; then
  git clone https://github.com/zsh-users/zsh-autosuggestions $_CONTAINER_USER_HOME/$ZSHPATH/custom/plugins/zsh-autosuggestions
fi

if echo "$PLUGINS" | grep -w -q "zsh-syntax-highlighting"; then
  git clone https://github.com/zsh-users/zsh-syntax-highlighting $_CONTAINER_USER_HOME/$ZSHPATH/custom/plugins/zsh-syntax-highlighting
fi

if echo "$PLUGINS" | grep -w -q "autoupdate"; then
  git clone https://github.com/TamCore/autoupdate-oh-my-zsh-plugins $_CONTAINER_USER_HOME/$ZSHPATH/custom/plugins/autoupdate
fi

if echo "$PLUGINS" | grep -w -q "autojump"; then
  git clone https://github.com/wting/autojump $_CONTAINER_USER_HOME/$ZSHPATH/custom/plugins/autojump
  apk --no-cache add python3
  su -c "cd $_CONTAINER_USER_HOME/$ZSHPATH/custom/plugins/autojump/ && SHELL=zsh && ./install.py" $_CONTAINER_USER
...
if echo "$PLUGINS" | grep -w -q "alias-tips"; then
  apk --no-cache add python3
  git clone https://github.com/djui/alias-tips $_CONTAINER_USER_HOME/$ZSHPATH/custom/plugins/alias-tips
fi

if echo "$PLUGINS" | grep -w -q "powerlevel10k"; then
  git clone https://github.com/romkatv/powerlevel10k.git $_CONTAINER_USER_HOME/$ZSHPATH/custom/themes/powerlevel10k
